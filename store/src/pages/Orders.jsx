    ...
    <div className="container page-shell">
      ...
      {loading ? (
        ...
        <div className="empty-card">
          ...
          <Link className="btn btn-primary" to="/products">Lihat koleksi</Link>
        </div>
      ) : (
          ...
          {orders.map(o=>(
            <article className="order-card" key={o.id}>
              <div className="order-head">
                ...
                <b className={`badge badge-${o.status}`}>{labels[o.status]}</b>
              </div>
              <div className="order-items">
                {Array.isArray(o.items) && o.items.map(i=>(
                  <div key={i.id}>
                    <span>{i.product_name} × {i.quantity}</span>
                    <b>{rupiah(Number(i.price)*Number(i.quantity))}</b>
                  </div>
                ))}
              </div>
              <div className="order-foot">
                <span>{o.payment_method==='cod'?'COD':o.payment_method==='ewallet'?'E-wallet':'Transfer bank'}</span>
